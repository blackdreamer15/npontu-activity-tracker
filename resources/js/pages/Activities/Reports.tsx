...
export default function ActivitiesReports({
    ...
    const doneCount = updates.filter((update) => update.status === 'done').length;
        ...
        <>
            ...
            <div className="space-y-6 p-4">
                <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">Filter activity history by custom date ranges.</p>
                </div>
                    ...
                    <CardContent>
                        {updates.length === 0 ? (
                            <div className="text-center">
                                <p className="text-sm text-muted-foreground">No updates found for this date range.</p>
                                <p className="text-xs text-muted-foreground mt-1">Try adjusting your date range.</p>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {updates.map((update) => (
                                    <div key={update.id} className="flex flex-col gap-3 rounded-lg border p-4 hover:bg-muted/50 transition-colors md:flex-row md:items-start md:justify-between">
                                        <div className="flex-1 min-w-0 space-y-1">
                                            <p className="font-semibold text-foreground">{update.activity.title}</p>
                                            <p className="text-xs text-muted-foreground">
                                                {update.user.name} on {update.updated_for_date}
                                            </p>
                                            {update.remark && (
                                                <p className="text-sm text-foreground mt-2">{update.remark}</p>
                                            )}
                                        </div>
                                        <Badge variant={update.status === 'done' ? 'default' : 'secondary'} className="whitespace-nowrap">
                                            {update.status}
                                        </Badge>
                                    </div>
                                ))}
